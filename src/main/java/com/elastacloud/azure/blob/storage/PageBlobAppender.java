...
public class PageBlobAppender
{
	 ...
	 */
	private void checkRollover() throws URISyntaxException, StorageException, InterruptedException
	{

		if (pages != null && pages.size() > 0 && (pages.get(pages.size() - 1).getEndOffset() + this.message.getBytes().length) >
		                                         maxSize)
		{
			//rename
			rename();
	...
	private void deriveWriteOffset() throws StorageException
	{
		...
		if (pages.size() > 0)
		{
			...
			ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream(PAGE_SIZE_MULTIPLE);

			blob.downloadRange(lastPage.getEndOffset() - PAGE_SIZE_MULTIPLE + 1,
			                   (long) PAGE_SIZE_MULTIPLE,
	 ...
	 */
	public Boolean rename() throws URISyntaxException, StorageException, InterruptedException
	{
		CloudPageBlob newBlob = blobcontainer.getPageBlobReference(this.logFileName + fileSuffix);
		if (!newBlob.exists())
			newBlob.create(maxSize);
		newBlob.startCopy(this.blob);
		boolean copyInProgress = true;
		while (copyInProgress)
		{
			newBlob.downloadAttributes();
			copyInProgress = (newBlob.getCopyState().getStatus() == CopyStatus.PENDING) ||
			                 (newBlob.getCopyState().getStatus() == CopyStatus.ABORTED);
